...
PROGRAM trajau
  ...
  complex*16, dimension(2) :: coef, coefdot,del2R, del2P
...
!              call aureset(p,q,coef,timestep,state,m,del2R,del2P,V,Vd,d,Vp,Vpd,CC,CC1)

!              !2016
!              gamma_r = 0.5*(-Vpd(3-state,3-state)+Vpd(state,state))* dble(del2R(3-state)-del2R(state))
!              gamma_c = abs(Vpd(3-state,state)*(del2R(3-state)-del2R(state)))
!              gamma_c = (gamma_r - 2 * gamma_c)*timestep
!              gamma_r = -gamma_r*timestep
!
!               write(8,*) 'delR'
!               write(8,*) del2R
!               write(8,*) '***********************************'
...
!********************
             ...
             !2013
             gamma_r = 0.5*timestep*real((-Vpd(state,state)+Vpd(3-state,3-state))*&
                      &(-delR(state,state) + delR(3-state,3-state)))
...
subroutine  aureset(p,q,coeff,timestep,state,m,del2R,del2P,V,Vd,d,Vp,Vpd,CC,CC1)
  implicit none
  real*8, intent(inout):: p, q, timestep, m
  integer, intent(inout):: state
  complex*16, intent(inout), dimension(2):: coeff
  complex*16, intent(inout), dimension(2):: del2P, del2R
  real*8, dimension(2,2) :: V, Vd, d, Vp, Vpd,CC,CC1
  !! Local
  complex*16 II,temp
  complex*16, dimension(2,2) :: W2, delF, W, H
  complex*16, dimension(2) ::  delFdiag
  ...
  W = matmul(matmul(CC1,exp(-II*Vd)),CC)

  W2 = matmul(matmul(CC,exp(II*Vd)),CC1)

  
  temp = Vpd(state,state)
  delF = -Vpd 
  delF(1,1) = delF(1,1) +temp
  delF(2,2) = delF(2,2) +temp
  delFdiag(1) = delF(1,1)
  delFdiag(2) = delF(2,2)
  
  do i = 1,2
      del2R(i) = del2R(i) + del2P(i)/m*timestep + 0.5*(delF(i,i)/m)*&
                              &(conjg(coeff(i))*coeff(i))*timestep**2
      del2P(i) = del2P(i) + 0.5*(delF(i,i))*(conjg(coeff(i))*coeff(i))*timestep
  end do

  !delF = matmul(matmul(W,delF),W2)
  
  !do i = 1,2
  !    del2P(i) = del2P(i) + 0.5*(delF(i,i))*(conjg(coeff(i))*coeff(i))*timestep
  !end do
  
!  del2R = matmul(matmul(W,W2),del2R)
!  del2P = matmul(matmul(W,W2),del2P)
